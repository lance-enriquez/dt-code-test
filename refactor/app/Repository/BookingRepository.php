...
class BookingRepository extends BaseRepository
{
    ...
    public function store($user, $data): array
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            if ($data['immediate'] == 'yes') {
                ...
                $data['due'] = $due_carbon->format(self::DATETIME_FORMAT);
                $data['immediate'] = 'yes';
            ...
            if ($consumer_type == 'rwsconsumer') {
                ...
                $data['job_type'] = 'paid';
				$data['b_created_at'] = date(self::DATETIME_FORMAT);
			}
            if (isset($due)) {
				$data['will_expire_at'] = TeHelper::willExpireAt($due, $data['b_created_at']);
			}

            $data['by_admin'] = data_get($data, 'by_admin', 'no');
            ...
            $data['job_for'] = [];
			$data['job_for'][] = $this->getGender($job->gender);
			$data['job_for'][] = $this->getCertified($job->certified);
	...
	{
		if (!empty($gender) && $job->gender == 'male') {
			return 'Man';
		} else if (!empty($gender) && $job->gender == 'female') {
			return 'Kvinna';
		}
		return $gender;
	}
	 ...
	 *
	 */
	private function getCertified($certified)
	{
    ...
    public function jobToData($job): array
    {
		...
		$gender = $job->gender;
		$job_certified = $job->certified;
		
		$job_for = [];
		$job_for[] = $this->getGender($gender);
		$job_for[] = $this->getCertified($job_certified);
    ...
    {
        $completeddate = date(self::DATETIME_FORMAT);
        ...
        $job = $job_detail;
        $job->end_at = date(self::DATETIME_FORMAT);
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    {
//        if (in_array($data['status'], ['pending', 'assigned']) && date(self::DATETIME_FORMAT) <= $job->due) {
        $old_status = $job->status;
        ...
        if ($data['status'] == 'pending') {
            $job->created_at = date(self::DATETIME_FORMAT);
            $job->emailsent = 0;
        ...
        */
        ...
        if ($cuser->is('customer')) {
            ...
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                $job->status = 'pending';
                $job->created_at = date(self::DATETIME_FORMAT);
                $job->will_expire_at = TeHelper::willExpireAt($job->due, date(self::DATETIME_FORMAT));
                $job->save();
//                Event::fire(new JobWasCanceled($job));
    ...
    public function bookingExpireNoAccepted()
    {
        ...
        if ($cuser && ($cuser->is('superadmin') || $cuser->is('admin'))) {
            ...
            if (isset($requestdata['job_type']) && $requestdata['job_type'] != '') {
                $allJobs = $allJobs->whereIn('jobs.job_type', $requestdata['job_type'])
								   ->where('jobs.status', 'pending')
								   ->where('jobs.ignore_expired', 0)
								   ->where('jobs.due', '>=', Carbon::now());
            }
...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
        ...
        if ($job['status'] != 'timedout') {
            ...
            $job['will_expire_at'] = TeHelper::willExpireAt($job['due'], date(self::DATETIME_FORMAT));
            $job['updated_at'] = date(self::DATETIME_FORMAT);
